public class IDs {
    ...
    public static int[] getAllOverlaps() {        
        ...
        int[] both = {overlaps, partialOverlaps};
        return both;
    }

    private static String[] splitStrings(String s1) {
        int l1 = s1.length();
        int split = 0;
        for (int i = 0; i < l1; i++) {
            if (s1.charAt(i) == '-') {
        ...
        String s1second = s1.substring(split + 1);

        String[] arr = {s1first, s1second};

        return arr;
    }

    private static boolean partialOverlaps(String s1, String s2) {
        
        int firstRange1 = Integer.parseInt(splitStrings(s1)[0]);
        int secondRange1 = Integer.parseInt(splitStrings(s1)[1]);
